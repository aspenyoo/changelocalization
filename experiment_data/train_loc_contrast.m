...
if sessionnum == 1;
    ...
    Screen('DrawText',windowPtr,'Four oriented blobs will flash up once, and all together again',textx,texty,[255 255 255]);
    ...
    reliabilities = [1 .4];
    im = makeGabor(stimLength, reliabilities(1));
    im2 = makeGabor(stimLength,reliabilities(2));
    StimPatch = Screen('MakeTexture',windowPtr,im);
    StimPatch2 = Screen('MakeTexture',windowPtr,im2);
    StimSize = size(im);
    yshift = 50;
    ...
    for istim = [2 4];
        cuesrcrect = [0 0 StimSize];
        destrect = CenterRectOnPoint(cuesrcrect,X(istim),Y(istim));
        Screen('DrawTexture',windowPtr,StimPatch2,cuesrcrect,destrect,180-deg1(istim));
    end
    Screen('Flip', windowPtr);
    pause(0.3);
    ...
    for iexamp = 1:nExamples;
        ...
        % draw one gabor
        deg1 = round(rand*180);
        im = makeGabor(stimLength, rand*0.7+0.3);
        StimPatch = Screen('MakeTexture',windowPtr,im);
        StimSize = size(im);
...
Screen('Flip', windowPtr);
Exp_ChangeLoc(subjid, 'Contrast', sessionnum, nTrialsPerCond,1);
